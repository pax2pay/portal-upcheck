	...
	async function createCard(providers: ProviderCode[]) {
		if (providers.length) {
			...
			try {
				...
				const usernameSelector = "#username input.sc-smoothly-0-input"
				await page.waitForSelector(usernameSelector, { timeout: 60000 })
				await page.type(usernameSelector, process.env.username ?? "")
				const passwordSelector = "#password input.sc-smoothly-0-input"
				...
				for (let i = 0; i < providers.length; i++) {
					...
					const accountSelector = "#createCardForm #accountSelector > smoothly-0-selector"
					await page.waitForSelector(accountSelector, { timeout: 60000 })
					await page.click(accountSelector)
					const selectedAccount = `#createCardForm #accountSelector > smoothly-0-selector> div > nav > smoothly-0-item:nth-child(1 of .${providers[i]})`
					await page.waitForSelector(selectedAccount, { timeout: 60000 })
					await page.click(selectedAccount)
					const cardTypeSelector = "#createCardForm #cardTypeSelector > smoothly-0-selector"
					...
					await frame?.waitForSelector("input.sc-smoothly-0-input", { timeout: 60000 })
					const csc = await frame?.$eval("input.sc-smoothly-0-input", (el: any) => el.value)
